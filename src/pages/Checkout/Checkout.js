...
function Checkout(props) {
    ...
    useEffect(() => {
        ...
        connection.on("loadDanhSachGheDaDat", (dsGheKhachDat) => {
            ...
            dsGheKhachDat = dsGheKhachDat.filter(item => item.taiKhoan !== userLogin.name);
    ...
    const clearGhe = function (event) {
        connection.invoke('huyDat', userLogin.name, props.match.params.id);
    }
    ...
    const renderSeats = () => {
        return danhSachGhe.map((ghe, index) => {
            ...
            if (userLogin.name === ghe.taiKhoanNguoiDat) {
            ...
            <div className="grid grid-cols-12">
                ...
                <div className="col-span-3">
                    <h3 className="text-green-400 text-center text-4xl">
                        {danhSachGheDangDat.reduce((tongTien, ghe, index) => tongTien += ghe.giaVe, 0).toLocaleString()} đ</h3>
                    <hr />
...
export default function CheckoutTab(props) {
    ...
    const operations = <Fragment>
        {!_.isEmpty(userLogin) ? <Fragment>
            <button onClick={() => { history.push('/admin') }}>
                <div className="items-center justify-center h-12 w-12 flex text-2xl ml-5 rounded-full bg-red-200">
                    {userLogin.taiKhoan.substr(0, 1)}
                </div>Hello ! {userLogin.taiKhoan}
            </button>
            <button onClick={() => { logout() }} className="text-blue-800">Đăng xuất</button>
        </Fragment> : ''}
    </Fragment>
    return <div className="p-5">
        <Tabs tabBarExtraContent={operations} defaultActiveKey="1" activeKey={tabActive} onChange={(key) => {
            ...
            <TabPane tab={<div className="flex text-center items-center justify-center">
                <NavLink to="/"><HomeOutlined style={{ marginLeft: 10, fontSize: 25 }} /></NavLink>
            </div>} key="3">
            </TabPane>
        </Tabs>
    </div>
                ...
                <div className="h-full flex items-center border-gray-200 border p-4 rounded-lg">
                    ...
                    <div className="flex-grow">
                        ...
                        <p>
                            ...
                            {ticket.danhSachGhe.map((ghe, index) => <span className="text-green-500 text-xl" key={index}> [{ghe.tenGhe}] </span>)}
                        </p>
                    </div>
                </div>
            </div>
